    ...
    private ParallelChords getParallelChords(EllipseQuarterArc arc2, EllipseQuarterArc arc1, boolean startOfArc2) {
        ...
        if (numberOfParallelChords < arc1HalfSize) {
            ...
            if (firstHalf) {
                ...
                for (int i = arc1HalfSize; i < arc1Indexes.length + arc1HalfSize; i++)
...
//                slope = ((float) arc2Point[1] - (float) arc1y) / ((float) arc2Point[0] - (float) arc1x);
    ...
    public static RotatedRect createContour(Ellipse ellipse) {
        ...
        for (float angle = 0; angle < 360; angle += 0.1) {
            float radians = angle * pi / 180;
            float x = (float) (ellipse.aAxis * Math.cos(radians) * Math.cos(ellipse.rho) + ellipse.bAxis * Math.sin(radians) * Math.sin(ellipse.rho));

 ...
 * detector.process();
 * BufferedImage edges = detector.getEdgeImage();
 * </code></pre>
 *
...
//            data[i * 3 + 2] = (byte) ((dataBuff[i] >> 16) & 0xFF);
     ...
     *
     * @param edgeImage expected (though not required) to be null
     */
...
//        return Math.round(0.299f * r + 0.587f * g + 0.114f * b);
    ...
    private void writeEdges(int pixels[]) {
        ...
        if (edgeImage == null) {
            edgeImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
        }
        edgeImage.getWritableTile(0, 0).setDataElements(0, 0, width, height, pixels);
    }
